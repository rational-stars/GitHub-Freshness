...
- Code 按钮使用当前主题的高亮背景色和字体色，对应颜色开关关闭时保留 GitHub 原生样式。
- 在 Code 按钮后新增 branch-clock 快捷入口，点击可直接打开设置面板。
- 改为 document-start 早期处理，保留 GitHub 原生 Skeleton，并在真实文件列表首次绘制前完成着色、格式化和排序。
- 移除仓库加载和页面跳转中的 350/500ms 延迟，改为微任务级处理和目标 MutationObserver。
- 支持 Turbo、PJAX、History API 和动态行更新，重复执行不会插入多个快捷按钮。
- 移除隐藏、淡入和二次揭示过程，Code 按钮和文件列表不会先消失再出现。

## v1.1.8 - 2026年7月13日

- 使用新的 branch-clock 图标，并为扩展工具栏、油猴元数据、设置标题和文档首页提供统一图标。
- 设置面板采用“午夜薄荷”配色，统一保存、导入成功和限流提醒的颜色。
- 修复设置标题图标加载及标题文字对齐问题，Chrome 扩展直接使用内置图标。
- 主题选项支持本地化显示：中文为自动、浅色、深色，英文为 Auto、Light、Dark。
- 当前语言切换后立即预览全部设置面板文案，保存后才持久化，取消不会修改本地语言。

## v1.1.7 - 2026年7月13日

- 新增 Manifest V3 Chrome 扩展版本，可直接从 `extension` 目录加载。
- Chrome 扩展设置使用同步存储，并通过工具栏图标打开设置面板。
- 新增 JSON 设置导入和导出；备份不包含 AWESOME token，导入时保留本地 token。
- 默认界面语言由自动改为中文，保留自动、中文和英文选项。
- 修复 SweetAlert2 无效位置参数产生的控制台错误。
- 修复 GitHub 文件夹与文件图标颜色被页面样式覆盖的问题。
- 优化设置面板间距、控件对齐及移动端布局。
- 扩展依赖改为本地资源，符合 Chrome 扩展内容安全策略。

## 🌟 **修改 bug**：
版本更新日志： v1.1.6 - 2026年7月13日

- 修复 GitHub 页面结构更新后仓库列表、搜索结果和 Awesome 项目链接无法正确识别的问题。
- 修复 Pickr 颜色选择器 CSS 被 GitHub CSP 拦截后显示异常的问题。
- 修复保存设置时 Chrome 将 AWESOME token 误判为网站密码并弹出保存密码的问题。
- 补充深色主题配置兜底，避免自动/深色主题下配置为空。
- 兼容中英文用户界面及 GitHub 英文状态下的搜索页更新时间。
- 设置面板新增当前语言选项，支持自动、中文、英文。
- 优化 Awesome API 请求：增加缓存、重复处理保护和 API 限流单次提醒。
- 降低默认控制台日志输出，保留 DEBUG 开关便于调试。
- 文档站新增英文文档路径 `/en/`。
